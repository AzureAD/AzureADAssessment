...
function Write-AppInsightsException {
    ...
    $result = Invoke-RestMethod -UseBasicParsing -Method Post -Uri $IngestionEndpoint -ContentType 'application/json' -Body ($AppInsightsTelemetry | ConvertTo-Json -Depth 6) -Verbose:$false -ErrorAction SilentlyContinue
}
